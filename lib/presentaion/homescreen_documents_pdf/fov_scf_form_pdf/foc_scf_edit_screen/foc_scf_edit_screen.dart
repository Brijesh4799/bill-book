...
class _FovScfFormEditScreen extends State<FovScfFormEditScreen> {
  ...
  void initState(){
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final billProvider = Provider.of<FocScfPdfProvider>(context, listen: false);
      billProvider.fetchfovscfById(id:widget.id );
    });
  }
  @override
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        bottomNavigationBar: SafeArea(
          child: Padding(
            padding: const EdgeInsets.all(12.0),
            child: Consumer<FocScfPdfProvider>(
              builder: (context, provider, child) {
                return ElevatedButton(
                  onPressed: () async {
                    HelperFunctions helper = HelperFunctions();
                    ...
                    // Make sure you have the correct paymentId or record id
                    String paymentId = widget.id ?? "";
                    if (paymentId.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text("FOV SCF ID is missing")),
                      );
                      return;
                    }

                    // Call the update method from provider
                    final updatedData = await provider.updatetwsById(paymentId);

                    if (updatedData != null) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text("FOV SCF form updated successfully")),
                      );
