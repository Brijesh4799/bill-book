...
import '../money_edit_screen/money_edit_screen.dart';
import '../money_receipt_pdf_screen/money_receipt_pdf_screen.dart';
import '../provider/money_receipt_pdf_provider.dart';

class MoneyReceiptPdf extends StatefulWidget {
  const MoneyReceiptPdf({super.key});
...
class _MoneyReceiptPdfState extends State<MoneyReceiptPdf> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(title: 'Money Receipt Pdf'),
      body: ListView(
        ...
        children: [
          ...
          SearchInputBox(),
          SizedBox(height: 10,),
          Consumer<MoneyReceiptPdfProvider>(
            builder: (context, provider, child) {
              final totalSurveys = provider.moneyList?.data?.length ?? 0;

              return Center(
                child: Text(
                  'Total Money Receipt : $totalSurveys',
...
class _MoneyCardState extends State<MoneyCard> {
  ...
  void initState() {
    ...
    // Fetch packing list data after widget is built
    Future.microtask(() {
      Provider.of<MoneyReceiptPdfProvider>(context, listen: false).fetchmoneyData();
    });
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<MoneyReceiptPdfProvider>(
      builder: (context, provider, child) {
        ...
        final moneyList = provider.moneyList?.data;
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.moveFrom ?? "N/A"}',style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold),),
                        ),
                        const Spacer(),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.moveTo ?? "N/A"}',style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold),),
                        ),
                      ],
                    ),
                   SizedBox(height: 5,),
                   Center(
                     child: Text('${item.formData?.receiptDate ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                   ),
                   /* SizedBox(height: 5,),
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.receiptNumber ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                        const Spacer(),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                    ...
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.name ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                        const Spacer(),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.phone ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                      ],
                    ),
                    SizedBox(height: 5,),
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.receiptAgainst ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                        const Spacer(),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.billNumber ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                      ],
                    ),
                    SizedBox(height: 5,),
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.billDate ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                        const Spacer(),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.moveFrom ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                      ],
                    ),
                    SizedBox(height: 5,),
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.moveTo ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                        const Spacer(),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.paymentType ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                      ],
                    ),
                    SizedBox(height: 5,),
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.receiptAmount ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                        const Spacer(),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.paymentMode ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                      ],
                    ),
                    SizedBox(height: 5,),
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.transactionNumber ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                        const Spacer(),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.branch ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
                      ],
                    ),
                    SizedBox(height: 5,),
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10),
                          child: Text('${item.formData?.remark ?? "N/A"}',style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                        ),
