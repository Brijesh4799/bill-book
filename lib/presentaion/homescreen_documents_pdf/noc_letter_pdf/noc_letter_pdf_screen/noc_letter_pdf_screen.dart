...
class NocPdfWebViewScreen extends StatefulWidget {
  final String id;
  const NocPdfWebViewScreen({Key? key, required this.id}) : super(key: key);

  @override
  _NocPdfWebViewScreenState createState() => _NocPdfWebViewScreenState();
}

class _NocPdfWebViewScreenState extends State<NocPdfWebViewScreen> {
  ...
  void initState() {
    ...
    Future.microtask(() async {
      try {
        ...
        print('📄 Noc PDF Subscription status: $isSubscribed');

        if (!isSubscribed) {
          print('Blocking screenshots because user is not subscribed (Noc PDF)');
          try {
            await ScreenProtector.preventScreenshotOn();
            print('✅ Screenshot protection enabled for Noc PDF');
          } catch (e) {
            print("⚠️ Failed to block screenshots: $e");
          }
        } else {
          print('Allowing screenshots because user is subscribed (Noc PDF)');
          try {
        ...
        print("⚠️ Noc PDF subscription check failed: $e");
      } finally {
        // Always fetch PDF regardless of subscription status
        context.read<NocLetterPdfProvider>().fetchnocPdf(widget.id);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'Noc PDF'),
      body: Consumer<NocLetterPdfProvider>(
